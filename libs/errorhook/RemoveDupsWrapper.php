...
class Debug_ErrorHook_RemoveDupsWrapper implements Debug_ErrorHook_INotifier
{
    ...
    private function _isExpired($hash)
    {
        ...
        $notExist = !file_exists($file);
        $timeExpired = @filemtime($file) < time() - $this->_period;

        return $notExist || $timeExpired;
    }
    ...
    private function _touch($hash, $errfile, $errline)
    {
        ...
        if (false === file_put_contents($file, "$errfile:$errline " . date('Y-m-d H:i:s'))) {
